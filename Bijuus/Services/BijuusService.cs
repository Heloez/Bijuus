...
public List<Bijuus> GetBijuus()
{
    PopularSessao();
    var Bijuus = JsonSerializer.Deserialize<List<Bijuus>>
        (_session.HttpContext.Session.GetString("Bijuus"));
    return Bijuus;
}
public List<caracteristicas> GetTipos()
{
    PopularSessao();
    var tipos = JsonSerializer.Deserialize<List<caracteristicas>>
        (_session.HttpContext.Session.GetString("caracteristicas"));
...
{
        Current = Bijuus.Where(p => p.Numero == Numero)
            .FirstOrDefault(),
        Prior = Bijuus.OrderByDescending(p => p.Numero)
            .FirstOrDefault(p => p.Numero < Numero),
        Next = Bijuus.OrderBy(p => p.Numero)
            .FirstOrDefault(p => p.Numero > Numero),
};
return Bijuus;
}
    ...
    if (string.IsNullOrEmpty(_session.HttpContext.Session.GetString("caracteristicas")))
{
        _session.HttpContext.Session
            .SetString("caracteristicas", LerArquivo(bijuusFile));
        _session.HttpContext.Session
            .SetString("caracteristicas", LerArquivo(caracteristicasFile));
    }
}
